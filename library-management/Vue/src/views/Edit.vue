...
<body class="p-6 font-poppins">
    ...
    <div class="max-w-md mx-auto">
        ...
        <input   type="submit" @click="updateBook" value="Update" class="bg-green-300 cursor-pointer hover:bg-green-600 hover:text-white  py-2 px-4 rounded duration-150">
    </div>
...
export default{
    ...
    methods:{
        getBookData(bookId){
            axios.get('http://localhost:8000/api/books/'+bookId+'/edit').then(res=>{
                this.model.book=res.data.book
            })
        },
        updateBook(){
            var mythis=this;
            axios.put('http://localhost:8000/api/books/'+this.bookId+'/edit',this.model.book)
            .then(res=>{
